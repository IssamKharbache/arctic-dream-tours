...
export default function BookingStepsBar({ step, t }: BookingStepsBarProps) {
  const steps = [
    { id: 1, label: t("steps.contactDetails"), icon: User },
    { id: 2, label: t("steps.checkout"), icon: CreditCard },
  ];
  return (
      ...
      <div className="max-w-2xl mx-auto">
        <div className="flex items-center justify-between relative">
          ...
          <div
            ...
            style={{
              width:
                step === 1 ? "0%" : step === 2 ? "calc(100% - 80px)" : "0%",
            }}
          ></div>
          {steps.map((s) => {
            const Icon = s.icon;
                  ...
                  <div className="mt-1">
                    {isCompleted && (
                      <span className="inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium bg-green-100 text-green-800">
                        {t("steps.status.completed")}
                      </span>
                    )}
                    {isActive && (
                      <span className="inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium bg-blue-100 text-blue-800">
                        {t("steps.status.current")}
                      </span>
                    )}
                    {isUpcoming && (
                      <span className="inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium bg-gray-100 text-gray-600">
                        {t("steps.status.upcoming")}
                      </span>
                    )}
                  </div>
                </div>
