...
export default function PromoBar() {
  ...
  return (
    <nav
      className={`${openPromobar ? "fixed" : "hidden"}
    top-0
    left-0
    w-full
    z-[60]
  ...
  `}
    >
      ...
      <div>
        {/* Aurora */}
        <div className="absolute pointer-events-none inset-0 opacity-40">
          <div className="absolute pointer-events-none -top-12 left-1/4 h-40 w-80 rounded-full bg-cyan-400 blur-3xl animate-pulse" />
          <div
            className="absolute pointer-events-none top-6 right-1/3 h-40 w-80 rounded-full bg-emerald-400 blur-3xl animate-pulse"
            style={{ animationDelay: "1s" }}
          />
        </div>

        {/* Grid */}
        <div className="absolute pointer-events-none inset-0 bg-[linear-gradient(rgba(255,255,255,.03)_1px,transparent_1px),linear-gradient(90deg,rgba(255,255,255,.03)_1px,transparent_1px)] bg-[size:40px_40px]" />
      ...
      <button
        onClick={() => {
          setOpenPromoBar(false);
        }}
        className="flex items-center justify-center text-white  h-8 w-8 mt-5 mr-4   rounded-full cursor-pointer "
      >
        <X />
      </button>
    </nav>
