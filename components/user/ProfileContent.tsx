...
const ProfileContent = ({ userData }: ProfileContentProps) => {
  ...
  const updateProfileMutation = useMutation({
    mutationFn: async (updatedData: {
      ...
      const res = await fetch(`${baseUrl}/api/user/updatedata/${userData.id}`, {
        method: "PUT",
    ...
    onError: (error: any) => {
      if (error.status === 409) {
        setProfileErrors((prev) => ({
          ...prev,
          email:
            "Email is already in use by another account. Try using another email address",
        }));
            ...
            <div key={field} className="space-y-2">
              ...
              {isEditingProfile ? (
                <>
                  ...
                  />
                  {profileErrors[field as keyof typeof profileErrors] && (
            ...
            ["currentPassword", "newPassword", "confirmPassword"].map(
              (field, idx) => (
                <div key={field} className="space-y-2">
                  ...
                  {passwordErrors[field as keyof typeof passwordErrors] && (
                    <p className="text-sm text-red-400">
                      {passwordErrors[field as keyof typeof passwordErrors]}
                    </p>
                  )}
